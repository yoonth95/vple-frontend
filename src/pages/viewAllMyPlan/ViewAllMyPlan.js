...
function ViewAllMyPlan() {
    ...
    const deletePlan = () => {
        let deleteUrl = `ec2-3-35-56-252.ap-northeast-2.compute.amazonaws.com:8080/auth/plan/${deleteId}`;
        let token = localStorage.getItem('token');

        axios.post(deleteUrl, deleteId,
            {
                headers: {
                    Authorization: token
                },
                        ...
                        <WrapCards>
                            {
                                myPlans.map((value, index) => {
                                    return (
                                        <WrapCard>
                                            <img className='delete-btn' src={DeleteBtn} onClick={() => setDeletePlanId(value.id, value.title)} />
                                            <img className='img-photo' src={value.image} />
